	...
	for {
		...
		case msg, ok := <-c.send: //將client端的訊息拿出來
			if !ok { //如果是false 代表client 可能斷線或關閉瀏覽器
				...
				c.conn.WriteMessage(websocket.CloseMessage, []byte{}) // 通知client端關閉訊息
				return
			}
			if err := c.conn.SetWriteDeadline(time.Now().Add(writeWait)); err != nil { //設定超時時間
			...
			if _, err = writer.Write(msg); err != nil { //發送訊息到client端
